...
class EventController extends Controller
{
    ...
    {
        $event = Event::where('id_event', 1)->first();
        return view("event.certificate", compact('event'));
    }
    ...
    {
        $validator = Validator::make($request->all(), [
            'image_certificate' => 'nullable|image|max:1024|mimes:jpg,jpeg,png',
            'certificate_name_x' => 'required|integer', // validasi untuk koordinat
            'certificate_name_y' => 'required|integer', // validasi untuk koordinat
        ]);
        ...
        if ($request->hasFile('image_certificate')) {
            $imageName = 'certificate' . $event->id_event . '-' . time() . '.' . $request->file('image_certificate')->extension();
            File::ensureDirectoryExists(public_path('img/event'));
            $request->file('image_certificate')->move(public_path('img/event/'), $imageName);
            if ($event->image_certificate != "" && file_exists(public_path('img/event/' . $event->image_certificate))) {
                unlink(public_path('img/event/' . $event->image_certificate));
            }
            $event->image_certificate = $imageName;
        }

        // Simpan koordinat X dan Y
        $event->certificate_name_x = $request->certificate_name_x;
        $event->certificate_name_y = $request->certificate_name_y;
        $event->save();

        return redirect()->back()->with("success", "E-Sertifikat berhasil diupdate");
    }
}
