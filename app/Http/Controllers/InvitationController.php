...
class InvitationController extends Controller
{
    ...
    public function guestRegister()
    {
        ...
        $currentRegistrations = \App\Models\Guest::count(); // Gunakan model Guest untuk menghitung pendaftar, bukan Invitation.
        // Invitation adalah tiket, Guest adalah pendaftar.

        // Cek status pendaftaran (jika ada kolom registration_status di tabel setting)
        if ($setting->registration_status == 0) { // Jika status pendaftaran non-aktif
            return view('register.form_close', compact('event', 'setting'));
        }

        // Cek apakah limit sudah tercapai
        // Pastikan registration_limit > 0 untuk mengaktifkan limit
        if ($setting->registration_limit > 0 && $currentRegistrations >= $setting->registration_limit) {
            return view('register.form_close', compact('event', 'setting'));
        }

        return view('register.form', compact('event', 'setting'));
    }
...
<body data-bgcolor="Body"
    ...
    /**
     * Membuat gambar sertifikat dengan nama peserta menggunakan GD Library.
     *
     * @param string $qrcode
     * @return string|null Path ke gambar sertifikat yang dihasilkan atau null jika gagal.
     */
    private function generateCertificateImage($qrcode)
    {
        $invt = Invitation::join('guest', 'guest.id_guest', '=', 'invitation.id_guest')
            ->where('qrcode_invitation', $qrcode)->first();
        $event = Event::where('id_event', 1)->first();

        if (!$invt || !$event || !$event->image_certificate) {
            return null;
        }

        $templatePath = public_path('img/event/' . $event->image_certificate);
        if (!file_exists($templatePath)) {
            return null;
        }

        // Buat resource gambar dari template
        $imageMime = mime_content_type($templatePath);
        switch ($imageMime) {
            case 'image/png':
        ...
        // Tentukan warna dan font
        $textColor = imagecolorallocate($image, 0, 0, 0); // Warna Hitam
        $fontPath = public_path('/fonts/Roboto-Bold.ttf');
        if (!file_exists($fontPath)) {
            return null;
        }

        $name = $invt->name_guest;
        $angle = 0;
        $fontSize = 22;

        $textBox = imagettfbbox($fontSize, $angle, $fontPath, $name);

        // Tengahkan teks secara horizontal pada gambar ukuran penuh
        $textWidth = $textBox[2] - $textBox[0];
        $imageWidth = imagesx($image);
        $x_centered = ($imageWidth - $textWidth) / 2;

        // Gunakan koordinat Y yang sudah diskalakan dari database (diatur melalui JS yang diperbarui)
        // Koordinat ini mewakili bagian ATAS teks yang diinginkan
        $y_top_scaled = $event->certificate_name_y;

        // Sesuaikan Y yang diskalakan untuk perataan baseline
        $y_baseline = $y_top_scaled + abs($textBox[7]);

        imagettftext($image, $fontSize, $angle, $x_centered, $y_baseline, $textColor, $fontPath, $name);

        // Simpan sertifikat yang dihasilkan
        $certificateDir = public_path('img/certificates');
        File::ensureDirectoryExists($certificateDir);
        $certificatePath = $certificateDir . '/' . $qrcode . '.jpg';
        imagejpeg($image, $certificatePath, 90);
        imagedestroy($image);

        return $certificatePath;
    }

    // Method untuk menampilkan view email sertifikat
    public function linkCertificateEmail($qrcode)
    {
        $invt = Invitation::join('guest', 'guest.id_guest', '=', 'invitation.id_guest')
            ->where('qrcode_invitation', $qrcode)->first();
        $event = Event::where('id_event', 1)->first();
        $setting = Setting::first();
        return view('link-guest.sendCertificateMail', compact('invt', 'event', 'setting'));
    }

    // Method baru untuk mengirim e-sertifikat
    public function sendCertificateEmail()
    {
        ...
        try {
            ...
            $mail->setFrom(env("MAIL_FROM_ADDRESS"), "E-Sertifikat " . myEvent()->name_event);
            $mail->addAddress($guestMail, $guestName);

            // Buat gambar sertifikat secara dinamis
            $certificatePath = $this->generateCertificateImage($guestQrcode);

            if ($certificatePath && file_exists($certificatePath)) {
                // Lampirkan gambar sertifikat ke email
                $mail->AddEmbeddedImage($certificatePath, 'certificate_image', 'certificate.jpg');
            } else {
                return redirect("invite")->with("error", "Gagal membuat gambar sertifikat. Periksa template dan file font.");
            }

            $mail->isHTML(true);
            $mail->Subject = "E-Sertifikat " . myEvent()->name_event;
            $mail->Body    = $this->linkCertificateEmail($guestQrcode)->render();

            $mail->send();
            $mail->SmtpClose();

            // Hapus file sertifikat sementara setelah email terkirim
            if (file_exists($certificatePath)) {
                unlink($certificatePath);
            }

            // Update status pengiriman sertifikat di database
            Invitation::where('qrcode_invitation', $guestQrcode)->update(['send_certificate_invitation' => 1]);

            $status = "success";
            $message = "Berhasil mengirim e-sertifikat";
        } catch (Exception $e) {
            $status = "error";
            $message = "Gagal mengirim e-sertifikat: " . $e->getMessage();
        }
        return redirect("invite")->with($status, $message);
    }
