                        ...
                        <table class="table table-striped display nowrap" id="table-1" style="width: 100%;">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        @if ($invitation->checkin_invitation)
                                        @if ($invitation->send_certificate_invitation != 1)
                                        <span class="text-warning font-italic">Waiting</span> &nbsp;
                                        <a class="btn btn-sm btn-success send-email" title="Kirim E-Sertifikat"
                                            data-email="{{ $invitation->email_guest }}"
                                            data-name="{{ $invitation->name_guest }}"
                                            href="{{ url('invite/send-certificate-email?guestQrcode=' . $invitation->qrcode_invitation . '&guestMail=' . $invitation->email_guest . '&guestName=' . $invitation->name_guest) }}">
                                            <i class="fa fa-award"></i> Send
                                        </a>
                                        @else
                                        <span class="text-success font-italic">Sent</span> &nbsp;
                                        <a class="btn btn-sm btn-success send-email" title="Kirim Ulang E-Sertifikat"
                                            data-email="{{ $invitation->email_guest }}"
                                            data-name="{{ $invitation->name_guest }}"
                                            href="{{ url('invite/send-certificate-email?guestQrcode=' . $invitation->qrcode_invitation . '&guestMail=' . $invitation->email_guest . '&guestName=' . $invitation->name_guest) }}">
                                            <i class="fa fa-award"></i> Resend
                                        </a>
                                        @endif
                                        @else
                                        <span class="btn btn-sm btn-outline-dark" style="cursor: not-allowed;">Belum Hadir</span>
                                        @endif
                                    </td>
